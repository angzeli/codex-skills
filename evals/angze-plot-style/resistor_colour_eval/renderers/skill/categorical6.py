"""Condition B: six unrelated categories using SKILL.md only."""
...
def main() -> None:
    ...
    views = common.scenario_views(dataset, "categorical6")

    with mpl.rc_context(rc=ANGZE_RC):
        figure, axis = plt.subplots(figsize=(8, 6))
        for view in views:
            colour = IDENTITY_COLOURS[view.series.identity]
            axis.plot(
                view.series.x,
                view.series.y,
                color=colour,
        ...
        output_stem = EVAL_ROOT / "outputs" / "B_skill" / "categorical6_B"
